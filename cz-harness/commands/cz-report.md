...
2. **Plan** — before writing anything, run the RTM orphan scan. Check all seven orphan classes:
   ...
   Also count: stale TC count (per `guard-rd-freeze` flags), `red_skipped` count (RDs where no valid red log preceded green, if any slipped through), `pending_verification` count (RDs currently flagged `pending_verification: true` in their `rd/*.md` frontmatter — a deferred red-green proof that is still owed, per `schemas/rd.schema.json`; list each by id with its `pending_verification_reason`, not just a count, since this is exactly the class of fact that must not get lost between now and whenever the real test run happens), and `src/**` files with no RD-ID annotation.
...
4. **Execute** — `agentops` writes `deliverables/RTM-$1.md` (full traceability table + orphan summary, with a `pending_verification` column/section listing every RD still flagged that way and its reason — an RD in this state looks identical to a properly proven one everywhere else, so the RTM is the one place that must always call it out), a telemetry rollup (throughput, mean red-to-green time, stall count, gate pass/fail rate), `deliverables/WEEKLY-$1.md` (narrative for stakeholders), and appends to `deliverables/CASE-STUDY.md` (durable record across weeks, not overwritten).

...
| **Tầng 2 — Model** | A `model-gateway` port (abstract interface, provider-agnostic) with OmniRoute as the reference adapter implementation; role→model-tier routing table so that, e.g., ba/sa roles route to a reasoning-tier model while boilerplate-generation routes to a cheaper tier |
...
| Bảo mật / Security | Present | Secrets deny-list, hazard-path auto-escalation, security-reviewer stage in the gate pipeline, OmniRoute lockdown (see `SECURITY-NOTES.md`) |

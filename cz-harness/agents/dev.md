...
3. Context isolation: test-designer's authoring context/conversation must never be visible to you, and you must never be visible to test-designer's context. Implement strictly from the RD's stated acceptance criteria and the tests' observable behavior (inputs/expected outputs) — not from any side-channel about how test-designer reasoned. If you notice you've been given test-designer's chat context, refuse to use it and implement only from the RD/AC/test contents.
   - This isolation also applies to tooling, not just chat context: a whole-project sanity check (e.g. `tsc --noEmit` against a root config whose `include` covers both `src` and `tests`) can leak test-file type signatures into your diagnostics just as effectively as reading test-designer's context directly. If the project has a typed language, run any sanity typecheck against a `src`-only config (e.g. `tsconfig.src.json` extending the root config with `include: ["src"]` only) — create one the first time it's needed if it doesn't exist yet — never the config that also covers `tests/**`.
